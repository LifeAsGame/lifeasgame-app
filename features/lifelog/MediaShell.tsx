...
function Select({ name, label, values, optional = false, disabled = false }: { name: string; label: string; values: readonly string[]; optional?: boolean; disabled?: boolean }) {
  return <label className="block text-xs" style={{ color: SAO.color.text.label }}>{label}<select name={name} aria-label={label} defaultValue="" required={!optional} disabled={disabled} style={INPUT_STYLE}><option value="">{optional ? "Preserve..." : "Select..."}</option>{values.map((value) => <option key={value}>{value}</option>)}</select></label>;
}
...
function Detail({ item, pending, update, remove }: { item: MediaInfo; pending: boolean; update: (body: MediaUpdateRequest) => Promise<boolean>; remove: () => Promise<boolean> }) {
...
export default function MediaShell() {
  ...
  return <div className="relative flex min-w-0 w-fit flex-row flex-nowrap items-center gap-3" data-testid="media-shell">
    ...
    <PanelFrame title="Media Detail" depth={0}>{media.detail ? <Detail item={media.detail} pending={pending} update={(body) => media.update(media.detail!.id, body)} remove={() => media.remove(media.detail!.id)} /> : <InfoCard>Select a Media source.</InfoCard>}</PanelFrame>
  </div>;
}
